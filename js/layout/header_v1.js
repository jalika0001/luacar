(function () {
    ...
    .wbs-model{
        ...
        width: 100%;
        height: 100vh;
        height: 100svh; /* svh accounts for mobile browser UI (address bar) so the sheet never renders behind it */
        left: 0;
        bottom: 0;
        display: flex;
        justify-content: center;
    ...
    @media screen and (max-width: 768px) {
       .wb-main-action .wbma-box{ gap: 0.5rem;}
    }
    @media screen and (max-width: 650px) {
        .web-header .row .btn-open-menu,
        .web-header .row .wbs-box .icon,
        ...
        .web-tablink.web-tablink-unactive{
    ...
    @media screen and (max-width: 540px) {
        .wbs-model .csc-box{grid-template-columns: repeat(2, 1fr);}
    }
    @media screen and (max-width: 450px) {
        .wbs-model .hd-box .sh-box{margin: 0rem 0.5rem;}
        .wbs-model .sg-box blockquote h2{
            font-size: 1rem;
        }
        .wbs-model .sg-box blockquote p{
    ...
    @media screen  and (max-width: 400px) {
        .wbs-model .hd-box .btn{padding: 0rem 0.5rem;}
        .wbs-model .csc-box{grid-template-columns: repeat(1, 1fr);}  
    }
    ...
    <div class="web-header">
        <div class="wbh-box df-s">
            ...
            <div class="row">
                <div class="wb-search" data-action="toggle-search-model">
                    <div class="wbs-box df-l">
                        ...
                        <input type="text" class="wbs-input" id="wbHeaderSearchInput" data-action="search-input" placeholder="品牌、型号、年份、参数，任意搜索，点我试试"/>
                    </div>
                </div>
            </div>
            <div class="row">
    ...
    <div class="wb-main-action">
        <div class="wbma-box">
            <div class="icon icon-ra icon-sm btn-action-wb-scroll-top" data-action="scroll-top">
            ...
            <div class="icon icon-ra icon-sm btn-action-customer-service" data-action="customer-service">
    ...
    <div class="web-tablink wbtl-s9">
        <div class="wbtl-box df-s">
            <!-- home webpage  -->
            <a href="/luacar" class="row active" data-route="v1,index,home">
            ...
            <!-- product filtter  -->
            <a href="/fillter-house" class="row" data-route="fitter,filter">
                <div class="icon icon-ra">
            ...
            <!-- customer service  -->
            <a href="/luacar-from" class="row" data-route="customer-service">
                <div class="icon icon-ra">
            ...
            <!-- new truck -->
            <a href="/new-truck" class="row" data-route="new-truck">
                <div class="icon icon-ra">
            ...
            <!-- go to rent house -->
            <a href="https://www.zuacar.com/zh" class="row" data-route="rent-house">
                <div class="icon icon-ra">
        ...
        <div class="wbsm-box">
            ...
            <div class="hd">
                <div class="hd-box df-s">
                    <div class="icon icon-la icon-sm x-smodel" data-action="toggle-search-model">
                    ...
                    <div class="sh-box df-s">
                        ...
                        <input type="text" id="wbsModelSearchInput" data-action="search-input" placeholder="品牌、型号、年份、参数，任意搜索，点我试试"/>
                    </div>
                    <button type="button" class="btn" data-action="search-submit">
                        搜索
                    </button>
                </div>
            </div>
    ...
    /* ─────────────── search: cache + fast query ─────────────── */

    const PREVIEW_LIMIT      = 6;    // inline preview shows at most this many
    const SEARCH_DEBOUNCE_MS = 220;  // wait for typing to pause before filtering
    const SEARCH_PAGE_URL    = '/search-item'; // full results page

    let carsCache        = null; // processed cars, kept in memory for the session
    let carsCachePromise = null; // in-flight fetch, shared across simultaneous callers
    let lastSearchQuery  = '';   // remembers the text behind the current preview

    // Precompute one lowercase string per car so each keystroke is a single
    // cheap substring check instead of re-reading/lowercasing several fields.
    ...
    function getCarsCached() {
        if (carsCache) return Promise.resolve(carsCache);
        if (carsCachePromise) return carsCachePromise;

        if (!window.API || typeof window.API.fetchCars !== 'function') {
            return Promise.resolve([]);
        }

        carsCachePromise = window.API.fetchCars()
            .then(function (raw) {
                const processed = window.API.processMultipleCars(raw || []);
                processed.forEach(function (car) { car._searchIndex = buildSearchIndex(car); });
                carsCache = processed;
                return processed;
            })
            .catch(function (err) {
            ...
            .finally(function () { carsCachePromise = null; });

        return carsCachePromise;
    }
    ...
    function carCardHTML(car) {
        ...
        return (
            '<li data-car-id="' + car.id + '" onclick="location.href=\'/car/detail?luacarId=' + car.id + '&luacarFieldId=' + car.fieldId + '\'">' +
                '<div class="csl-box">' +
                    '<div class="img"><img src="' + car.imageUrl + '" class="img-c" alt="' + escapeHtml(title) + '"/></div>' +
                    '<div class="text">' +
                        '<h2>' + escapeHtml(title) + '</h2>' +
                        '<div class="price"><span>' + escapeHtml(car.priceDisplay) + '</span></div>' +
                    '</div>' +
                    '<a href="/car/detail?luacarId=' + car.id + '&luacarFieldId=' + car.fieldId + '" class="btn">查看详情</a>' +
                '</div>' +
            '</li>'
        );
    }

    function renderCarSuggestionList(cars) {
        const list = document.getElementById('carSuggestionList');
        if (!list) return;

        if (!cars.length) {
            list.innerHTML = '<li class="wbs-empty">暂无匹配车辆，换个关键词试试</li>';
            return;
        }
    ...
    function toggleMoreButton(show) {
        const moreBox = document.getElementById('carSuggestionMore');
        if (!moreBox) return;
        moreBox.style.display = show ? '' : 'none';
    }

    // Default: .cs stays in its normal position (after slt/sg), heading says "为您推荐".
    // While actively searching: .cs jumps to right under the search box, heading
    // switches to "搜索结果" so it's clear these are results, not recommendations.
    function setSearchModeUI(isSearching, query) {
        const modal = document.getElementById('wbsModel');
        if (!modal) return;

        const hd = modal.querySelector('.hd');
        const cs = modal.querySelector('.cs');
        const ft = modal.querySelector('.ft');
        const heading = modal.querySelector('.csh h2');
        const box = modal.querySelector('.wbsm-box');
        if (!hd || !cs || !box) return;

        if (isSearching) {
            if (hd.nextElementSibling !== cs) {
                hd.insertAdjacentElement('afterend', cs);
            }
            if (heading) {
                heading.textContent = query ? '搜索结果："' + query + '"' : '搜索结果';
            }
        } else {
            if (ft && cs.nextElementSibling !== ft) {
                box.insertBefore(cs, ft);
            }
            if (heading) heading.textContent = '为您推荐';
        }
    }

    // Collects up to (limit + 1) matches so we can tell "is there more?"
    // without scanning/storing the full result set every keystroke.
    function collectMatches(cars, predicate, limit) {
        const out = [];
        for (let i = 0; i < cars.length && out.length <= limit; i++) {
            if (predicate(cars[i])) out.push(cars[i]);
        }
        return out;
    }
    ...
    function performSearch(query) {
        ...
        setSearchModeUI(!!q, raw.trim());

        getCarsCached().then(function (cars) {
            let collected;
            if (!q) {
                collected = collectMatches(cars, function (c) { return c.webStatus; }, PREVIEW_LIMIT);
            } else {
                collected = collectMatches(cars, function (c) { return c._searchIndex.indexOf(q) !== -1; }, PREVIEW_LIMIT);
            }

            const hasMore = collected.length > PREVIEW_LIMIT;
            renderCarSuggestionList(collected.slice(0, PREVIEW_LIMIT));
    ...
    // Keep header input and modal input showing the same value
    function syncSearchInputs(value) {
        document.querySelectorAll('[data-action="search-input"]').forEach(function (input) {
            if (input.value !== value) input.value = value;
        });
    }

    // Full results live on a separate page; both the search button and the
    // "see more" button send the user there with the query in the URL.
    function goToSearchPage(query) {
        const q = (query || '').trim();
        window.location.href = SEARCH_PAGE_URL + '?search=' + encodeURIComponent(q);
    }
    ...
    /* ─────────────── hide header + tablink while scrolling down ─────────────── */
    // YouTube-style behavior: scrolling down hides both the sticky header and
    // the bottom tab bar; scrolling up (in either direction, anywhere on the
    // page) brings them back immediately. A small threshold avoids flicker
    // from tiny/jittery scroll events (e.g. iOS bounce, trackpad micro-scrolls).
    let lastScrollYForNav = window.scrollY;
    const SCROLL_NAV_THRESHOLD = 5;

    function checkHeaderTablinkScroll() {
        const header = document.querySelector('.web-header');
        const tablink = document.querySelector('.web-tablink');
        const currentY = window.scrollY;
        const delta = currentY - lastScrollYForNav;

        if (currentY <= 0) {
            // Always show once back at the very top
            if (header) header.classList.remove('web-tablink-unactive');
            if (tablink) tablink.classList.remove('web-tablink-unactive');
        } else if (delta > SCROLL_NAV_THRESHOLD) {
            // Scrolling down -> hide
            if (header) header.classList.add('web-tablink-unactive');
            if (tablink) tablink.classList.add('web-tablink-unactive');
        } else if (delta < -SCROLL_NAV_THRESHOLD) {
            // Scrolling up -> show
            if (header) header.classList.remove('web-tablink-unactive');
            if (tablink) tablink.classList.remove('web-tablink-unactive');
    ...
    function syncActiveTab() {
        const rows = document.querySelectorAll('.wbtl-s9 .wbtl-box .row');
        if (!rows.length) return;

        const currentSlug = lastPathSegment(window.location.pathname);
        const fileNoExt = currentSlug.replace(/\.[^./]+$/, '');
        ...
        rows.forEach(function (row) {
            ...
            if (href) {
                ...
                const linkSlug = lastPathSegment(linkURL.pathname);
                isMatch = sameOrigin && linkSlug !== '' && linkSlug === currentSlug;
            }

            if (!isMatch && !href && row.hasAttribute('data-route')) {
                hasRouteInfo = true;
                const routes = row.getAttribute('data-route')
                    .split(',')
                    .map(function (r) { return r.trim().toLowerCase(); })
                    .filter(Boolean);

                isMatch = routes.indexOf(fileNoExt) !== -1 ||
                          routes.indexOf(currentSlug) !== -1 ||
                          (fileNoExt === '' && (routes.indexOf('index') !== -1 || routes.indexOf('home') !== -1));
            }
    ...
    function bindHeaderEvents(root) {
        root.addEventListener('click', function (event) {
            ...
            // Search modal opened/closed here (both the search bar and the ✕ icon
            if (event.target.closest('[data-action="toggle-search-model"]')) {
                const modal = document.getElementById('wbsModel');
                if (modal) modal.classList.toggle('active');

                const modalInput = document.getElementById('wbsModelSearchInput');
                performSearch(modalInput ? modalInput.value : '');

                if (modal && modal.classList.contains('active') && modalInput) {
                    // Wait a frame so the modal has finished becoming visible/laid out
            ...
            if (event.target.closest('[data-action="scroll-top"]')) {
                window.scrollTo({ top: 0, behavior: 'smooth' });
                return;
            }

            // Popular search term clicked -> fill input and search immediately
            const popularItem = event.target.closest('.slt-box li');
            if (popularItem) {
                const text = popularItem.textContent.trim();
                syncSearchInputs(text);
                performSearch(text);
                return;
            }

            // "See more" — go to the full search results page with the current query
            if (event.target.closest('[data-action="search-see-more"]')) {
                goToSearchPage(lastSearchQuery);
                return;
            }

            // Explicit search submit -> go to the full search results page
            if (event.target.closest('[data-action="search-submit"]')) {
                const modalInput = document.getElementById('wbsModelSearchInput');
                goToSearchPage(modalInput ? modalInput.value : lastSearchQuery);
                return;
            }
        ...
        // Search input (debounced, cached — fast on every keystroke)
        ...
        // Escape key closes lang dropdown; Enter submits the search (same as the button)
        root.addEventListener('keydown', function (event) {
            if (event.key === 'Escape') closeLang();
            if (event.key === 'Enter' && event.target.matches('[data-action="search-input"]')) {
        ...
        window.addEventListener('scroll', checkHeaderTablinkScroll, { passive: true });
    }

    /* ─────────────── init ─────────────── */

    /* ─────────────── prefill search inputs from URL ─────────────── */
    // If the page was loaded with ?search=xxx (e.g. arriving on search-item.html
    // from a header search), show that text back in both search boxes
    // (#wbHeaderSearchInput and #wbsModelSearchInput) instead of leaving them empty.
    function prefillSearchInputFromURL() {
    ...
    function initHeader(options) {
        ...
        checkHeaderTablinkScroll();
        syncActiveTab();
        prefillSearchInputFromURL();
    }
