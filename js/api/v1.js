...
if (typeof API_CONFIG === 'undefined') {
    var API_CONFIG = {
        BASE_URL: 'https://license.fi855.com/en',
        ENDPOINTS: {
...
/**
 * 筛选出"出售"状态的车辆
 * 对应 Odoo 自定义筛选: [("x_studio_selection_field_85p_1i5mvoku9", "=", "出售")]
 * @param {Array} cars - 原始车辆数据数组
 * @returns {Array} 只包含出售车辆的数组
 */
function filterForSaleCars(cars) {
    const filtered = cars.filter(car => car.x_studio_selection_field_85p_1i5mvoku9 === '出售');
    console.log(`API调试 - 出售筛选: 原始 ${cars.length} 条 → 出售 ${filtered.length} 条`);
    return filtered;
}
    ...
    try {
        ...
        for (let result of possibleResults) {
            if (Array.isArray(result) && result.length > 0) {
                ...
                return filterForSaleCars(result);
            }
        }
